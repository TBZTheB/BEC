...
public class BoundaryTestCaseGenerator2 {
        ...
        cu.accept(new ASTVisitor() {
            ...
            public boolean visit(MethodDeclaration node) {
                ...
                for (Object parameter : parameters) {
                    ...
                    System.out.println("Parameter: " + svd.getName());
                }
    ...
    private static void generateTestCases(MethodDeclaration method) {
        ...
        List<List<Object>> allBoundaryValues = new ArrayList<>();
        List<String> parameterNames = new ArrayList<>();
        List<String> parameterTypes = new ArrayList<>();
        Map<String, Object> averageValues = new HashMap<>();
        ...
        for (Object parameter : parameters) {
            ...
            List<Object> boundaryValues = extractBoundaryValues(method.getBody(), name, type);
            allBoundaryValues.add(boundaryValues);

            // Xác định giá trị trung bình
            Object averageValue = calculateAverageValue(boundaryValues, type);
            averageValues.put(name, averageValue);
        }

        Set<String> uniqueTestCases = new HashSet<>();
        generateBoundaryCombinations(method.getName().toString(), parameterNames, parameterTypes, allBoundaryValues, averageValues, uniqueTestCases);
        System.out.println("Method: " + method.getName());
        System.out.println("Unique test cases: " + uniqueTestCases.size());
        // Write unique test cases to file
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("BoundaryTestCases.txt"))) {
        ...
        switch (type) {
            ...
            case "float":
                ...
                switch (operator) {
                    ...
                    case ">=":
                        boundaryValues.add(floatValue);
                        boundaryValues.add(floatValue - 0.1f);
                        break;
                }
                break;
        }
    ...
    private static Object calculateAverageValue(List<Object> boundaryValues, String type) {
        if (boundaryValues.isEmpty()) {
            return determineDefaultAverage(type);
        }
        double sum = 0;
        for (Object value : boundaryValues) {
            sum += ((Number) value).doubleValue();
        }
        double average = sum / boundaryValues.size();
        switch (type) {
            case "int":
                return (int) average;
            case "double":
    ...
    private static Object determineDefaultAverage(String type) {
        switch (type) {
            case "int":
                return 50; // Giá trị trung bình giả định cho int
            case "double":
                return 50.0; // Giá trị trung bình giả định cho double
            case "short":
                return (short) 50; // Giá trị trung bình giả định cho short
            case "long":
                return 50L; // Giá trị trung bình giả định cho long
            case "float":
                return 50.0f; // Giá trị trung bình giả định cho float
            default:
                return null;
        }
    }

    private static void generateBoundaryCombinations(String methodName, List<String> parameterNames, List<String> parameterTypes, List<List<Object>> allBoundaryValues, Map<String, Object> averageValues, Set<String> uniqueTestCases) {
        int numParameters = parameterNames.size();

        // Tạo tổ hợp các giá trị biên và giá trị trung bình
        for (int i = 0; i < numParameters; i++) {
            for (Object boundaryValue : allBoundaryValues.get(i)) {
                ...
                for (int j = 0; j < numParameters; j++) {
                    ...
                    if (j == i) {
                        sb.append(parameterNames.get(j)).append(" = ").append(boundaryValue);
                    } else {
                        sb.append(parameterNames.get(j)).append(" = ").append(averageValues.get(parameterNames.get(j)));
                    }
                }
                sb.append(")");
                uniqueTestCases.add(sb.toString());
            }
        }
    }
}
